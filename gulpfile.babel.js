 ...
 * @link http://www.browsersync.io/docs/1options/
 *
 ...
 * Task: `scripts`.
 *
 * Concatenate and uglify plugin JS scripts.
 *
 * This task does the following:
 *     1. Gets the source folder for JS plugin files
 *     2. Concatenates all the files and generates plugin.js
 *     3. Renames the JS file with suffix .min.js
 *     4. Uglifes/Minifies the JS file and generates main.min.js
 */
gulp.task('jsReq', () => {
	return gulp
		.src(config.jsPluginSRC, {since: gulp.lastRun('jsReq')}) // Only run on changed files.
		.pipe(plumber(errorHandler))
		.pipe(
			babel({
				presets: [
		...
		.pipe(remember(config.jsPluginSRC)) // Bring all files back to stream.
		.pipe(concat(config.jsPluginFile + '.js'))
		.pipe(lineec()) // Consistent Line Endings for non UNIX systems.
		.pipe(gulp.dest(config.jsPluginDestination))
		.pipe(
			rename({
				basename: config.jsPluginFile,
				suffix: '.min'
			})
		)
		.pipe(uglify())
...
gulp.task('scripts', () => {
	return gulp
		.src(config.jsSRC, {since: gulp.lastRun('scripts')}) // Only run on changed files.
		.pipe(plumber(errorHandler))
		.pipe(
			babel({
				presets: [
 ...
 * Task: 'requirejs'
 *
 * Concatenates JS files required
 * */
...
gulp.task(
	'default',
	gulp.parallel('styles', 'editorStyles', 'scripts', 'jsReq', 'images', browsersync, () => {
		gulp.watch(config.watchPhp, reload); // Reload on PHP file changes.
		gulp.watch(config.watchStyles, gulp.parallel('styles')); // Reload on SCSS file changes.
		gulp.watch(config.watchJs, gulp.series('scripts', reload)); // Reload on vendorsJS file changes.
