...
$(window).on('load resize', function () {
  ...
  var brandWidth = $('.navbar-brand').outerWidth();
  var containerWidth = $('.navbar > .container').width();
  var contentHeight = $('.wrapper').outerHeight();
  var footerHeight = $('footer').outerHeight();
  var mastheadRowHeight = $('.jumbotron .row').outerHeight();
  var mqxs = window.matchMedia('screen and ( max-width: 767px )');
  var navbarWidth = $('.navbar-collapse').outerWidth();
  var toggleWidth = $('.navbar-toggle').outerWidth();
...
$(window).on('load resize', function sidebarFix() {
  ...
  var headingText = $('.sidebar-navigation > h2, .sidebar-navigation > button').text(); // clear the timeout so we can start fresh

  clearTimeout(timer);
  timer = setTimeout(function () {
    if (!mqsm.matches) {
      $('.sidebar-navigation').removeClass('dropdown'); // Replace the button with a proper heading element

      $('.sidebar-navigation > button').replaceWith('<h2>' + headingText + '</h2>'); // Fix the menu

      $('.sidebar-navigation > ul').removeClass('dropdown-menu').removeAttr(' aria-labelledby');
    } else {
      // wrap the list in a container div
      $('.sidebar-navigation').addClass('dropdown'); // Replace the class in the dropdown menu

      $('.sidebar-navigation > ul').addClass('dropdown-menu').attr('aria-labelledby', 'sidebar-nav-btn'); // Change the heading to a button
